...
namespace SolidWorksFaceManager
{
    public class FeatureFace
    {
        ...
        {
            double[] UVBounds = face.GetUVBounds();
            ...
            double[] param = surface.Evaluate(centerU, centerV, 0, 0);
        ...
        public Point3D[] GetVertixs() {       
            ...
            if (edges != null)
            {
                foreach (Edge edge in edges)
                {
                    ...
                    double[] start = edge.IGetStartVertex().GetPoint();
                    double[] end = edge.IGetEndVertex().GetPoint();
